...
int main() {
  ...
  std::ofstream reynolds_num_vs_L2_error_csv("./couette_flow_compressible_RK4/Re_vs_L2__u_wall_E-2.csv");
  reynolds_num_vs_L2_error_csv << "Re,L2" << std::endl;
  reynolds_num_vs_L2_error_csv << std::endl;

  /* Here, we run over each point per decade (3, to go from u_wall_top = 1.0e(OOM) to u_wall_top = 19.5e(OOM); where OOM is the order of magnitude set by hand). */
  for (int current_pt_in_decade = 0; current_pt_in_decade < 3; current_pt_in_decade++) {
    ...
    u_wall_top = (u_wall_top_reference + 3.0 * current_pt_in_decade) * 10e-2;
    Reynolds_num = (y_len * u_wall_top)/kinemat_visc_FECN;
    ...
    // start with FECN to populate first timestep
